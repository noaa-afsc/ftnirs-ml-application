...
Both inference and fine-tuning will flag if a column was NULLIFIED in the last training event, meaning, the biological/geospatial/temporal data was named and included some point in the model training, but in the latest round was given a dummy value to deprioritize it for retraining. These can be added back in as desired for fine-tuning, but consume a column for further retraining. For inference, a column currently nullified shouldn't be used except for experimental reasons, and care should be taken when reintroducing it with fine-tuning. 
